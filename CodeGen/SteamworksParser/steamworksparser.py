...
class Parser:
    ...
    def __init__(self, folder):
        ...
        # 我们希望这个功能能应用于“自动生成”模块，但可能不会将其应用于其他任何地方。
    ...
    def parse(self, s):
        for linenum, line in enumerate(s.lines):
            ...
            # 注释会从行中删除，通常会留下空行，因此我们在解析注释后执行此操作
            if not s.line:
                continue
    ...
    def parse_comments_singleline(self, s):
        ...
        if match:
            s.linecomment = s.line[match.end():]
            s.line = s.line[:match.start(1)]

            s.rawlinecomment = re.match(r'.*?(\s*//.*)', s.originalline).group(1).rstrip()
    ...
    def parse_typedefs(self, s):
        ...
        # 跳过我们当前不支持的 typedefs，到目前为止，它们都是函数指针。
        ...
        # 目前跳过 typedef struct ValvePackingSentinel_t
    ...
    def parse_constants(self, s):
        ...
        # 目前会跳过一个错误的函数定义，该定义中新的一行中的第一个参数以“const”开头。例如：
        # void func(void arg1,
        #    const arg2) = 0;
    ...
    def parse_enums(self, s):
        ...
        # 实际上是一个常量，例如：enum { k_name = value };
        if "};" in s.linesplit:
            # Skips lines like: "enum { k_name1 = value, k_name2 = value };"
            # 跳过像 "enum { k_name1 = value, k_name2 = value };" 这样的行
            # Currently only skips one enum in CCallbackBase
            # 目前仅跳过 CCallbackBase 中的一个枚举
            if "," in s.line:
                return
    ...
    def parse_structs(self, s):
        ...
        # 跳过 SteamIDComponent_t 和 GameID_t，它们是 CSteamID/CGameID 的一部分
        if s.scopeDepth != 0:
            return
    ...
    def parse_interfaces(self, s):
        if s.line.startswith("class ISteam"):
            comments = self.consume_comments(s)
            # Ignore Forward Declares and Matchmaking Responses
            # 忽略前向声明和匹配响应
            if s.linesplit[1].endswith(';') or s.linesplit[1].endswith("Response"):  
                return
    ...
    def parse_interface_functions(self, s):
        ...
        # 跳过不以 virtual 开头的行，除非我们当前正在解析函数
        ...
        for i, token in linesplit_iter:
            ...
            if s.funcState == 2:  # Args
                ...
                if token.startswith(")"):  # Like f( void arg ")"
                    if args:
                        TEST = 1
                        # TODO: Cleanup, I don't even know what the fuck is going on here anymore.
                        # TODO: 清理工作开始了，我现在甚至都不清楚这里到底发生了什么。
                        TEST2 = 0  
                    ...
                    if Settings.warn_spacing:
                        ...
                        printWarning("函数中在右括号和第一个参数之间缺少空格.", s)
