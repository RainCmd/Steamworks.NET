...
namespace Steamworks {
	public static class SteamInventory {
		...
		/// <para>结果序列化和认证</para>
		/// <para>序列化结果集包含一个短签名，无法在不同的游戏会话中被伪造或重放。结果集可以本地客户端序列化，通过你的游戏网络传输给其他玩家，并由远程玩家反序列化。 这种方式是一种安全的防止黑客谎报拥有稀有/高价值物品的方法。 将带有签名字节的结果集序列化到输出缓冲区。 将输出缓冲区设置为 NULL 以通过 punOutBufferSize 获取所需的尺寸。 序列化结果的大小取决于正在序列化的项目数量。 当安全地将物品传输到其他玩家时，建议首先使用“GetItemsByID”创建最小的结果集。 结果集内置时间戳，在经过一小时后将被视为“过期”。 请参阅 DeserializeResult 以获取过期处理。</para>
		/// </summary>
		...
		/// <para>AddPromoItem() / AddPromoItems() 是 GrantPromoItems() 的受限版本。而不是扫描所有有效的促销项目，检查仅限于单个项目定义或一组项目定义。这在你的游戏有自定义 UI 来向用户显示特定促销项目时非常有用。</para>
		/// </summary>
