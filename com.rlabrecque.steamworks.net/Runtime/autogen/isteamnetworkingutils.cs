...
namespace Steamworks {
	public static class SteamNetworkingUtils {
		...
		/// <para>如果缺乏足够近期的数据，则返回false。在这种情况下，会启动ping测量，如果它尚未激活。 (您无法重启正在进行的测量。)</para>
		...
		/// <para>从此主机到指定数据中心的最佳可用中继路由的延迟（ping）是多少？</para>
		/// </summary>
		...
		/// <para> your list.</para>
		/// <para>获取所有 POP ID 列表。返回填充到列表中的条目数量。</para>
		/// </summary>
