...
namespace Steamworks {
	public static class SteamUserStats {
		...
		/// <para>获取成就状态，以及如果已解锁的时间（如果已解锁）。如果返回值是true，但解锁时间为零，则意味着该成就是在Steam开始跟踪成就解锁时间（2009年12月）之前解锁的。时间以1970年1月1日至今的秒数计算。</para>
		/// </summary>
		...
		/// <para>存储当前数据到服务器，会收到一个回调，当设置成功时，以及一个回调，用于每个新成就。</para>
		...
		/// <para>Achievement / GroupAchievement 相关的元数据获取成就图标，这是一个用于在 ISteamUtils::GetImageRGBA() 中使用的句柄，如果未设置则返回 0。返回值 0 可能表示我们仍在获取数据，您可以等待 UserAchievementIconFetched_t 回调，该回调将通知您数据已准备好。如果回调仍返回 0，则表示对于指定的成就未设置图像。</para>
		/// </summary>
		...
		/// <para>成就进度 - 触发成就进度回调，就只是这样。使用 N/N 的进度调用此方法不会设置成就，游戏必须仍然执行此操作。</para>
		/// </summary>
		...
		/// <para>用于迭代成就。一般来说，游戏不应该需要这些函数，因为它们应该有已有的成就列表编译到它们中。</para>
		/// </summary>
		...
		/// <para>获取成就名称 iAchievement 在 [0, GetNumAchievements)</para>
		/// </summary>
		...
		/// <para>朋友的统计数据和成就下载统计信息对于用户返回一个 UserStatsReceived_t received，如果其他用户没有统计数据，UserStatsReceived_t.m_eResult 将被设置为 k_EResultFail，这些统计数据不会自动更新，您需要再次调用 RequestUserStats() 来刷新任何数据。</para>
		/// </summary>
		...
		/// <para>查看 GetAchievementAndUnlockTime 上的备注。</para>
		/// </summary>
		...
		/// <para>排行榜函数会向 Steam 后端请求排行榜，如果尚未创建则会创建它。这个调用是异步的，结果会返回在 LeaderboardFindResult_t 中。</para>
		/// </summary>
		...
		/// <para> This call is asynchronous, with the result returned in LeaderboardFindResult_t</para>
		/// <para>以上内容翻译为： 不会创建排行榜，如果未找到则不创建。此调用是异步的，结果将在 LeaderboardFindResult_t 中返回。</para>
		/// </summary>
		...
		/// <para>请求 Steam 后端获取一组排行榜数据。这个调用是异步的，结果会通过 LeaderboardScoresDownloaded_t 返回。LeaderboardScoresDownloaded_t 会包含一个句柄，用于从 GetDownloadedLeaderboardEntries() 中拉取结果。你可以请求比实际存在的条目数量更多的条目，它会返回存在的条目数量。k_ELeaderboardDataRequestGlobal 请求来自排行榜的全部数据行，nRangeStart & nRangeEnd 在 [1, TotalEntries] 范围内。k_ELeaderboardDataRequestGlobalAroundUser 请求当前用户周围的数据行，nRangeStart 为负数，例如 DownloadLeaderboardEntries( hLeaderboard, k_ELeaderboardDataRequestGlobalAroundUser, -3, 3 ) 将返回 7 行，3 行在用户之前，3 行在用户之后。</para>
		/// </summary>
		...
		/// <para>下载排行榜条目对于任意一组用户 - ELeaderboardDataRequest 是 k_ELeaderboardDataRequestUsers。如果用户没有排行榜条目，他们将不会包含在结果中。最多可以下载 100 个用户，并且一次只能进行一个突出显示的调用。</para>
		/// </summary>
		...
		/// <para>使用从 0 到 LeaderboardScoresDownloaded_t::m_cEntryCount 的 for 循环获取所有下载的条目，例如：void OnLeaderboardScoresDownloaded( LeaderboardScoresDownloaded_t *pLeaderboardScoresDownloaded )</para>
		/// <para>for ( int index = 0; index < pLeaderboardScoresDownloaded->m_cEntryCount; index++ )</para>
		/// <para>LeaderboardEntry_t leaderboardEntry; int32 details[3];		// 我们知道这是我们之前存储的多少 GetDownloadedLeaderboardEntry( pLeaderboardScoresDownloaded->m_hSteamLeaderboardEntries, index, &leaderboardEntry, details, 3 ); assert( leaderboardEntry.m_cDetails == 3 );</para>
		...
		/// <para>将用户分数上传到 Steam 后端。此调用是异步的，结果将在 LeaderboardScoreUploaded_t 详情中返回。pScoreDetails 指向一个 int32 数组，cScoreDetailsCount 是该列表中 int32 的数量。</para>
		/// </summary>
		...
		/// <para>将用户生成的内容附加到用户的排行榜条目。hContent 是一个句柄，指向使用 ISteamUserRemoteStorage::FileShare() 分享的由用户生成的的内容。此调用是异步的，结果将返回在 LeaderboardUGCSet_t 中。</para>
		/// </summary>
		...
		/// <para>Steam 请求获取每个游戏全球范围内已获得的成就百分比数据。该调用是异步的，结果将以 GlobalAchievementPercentagesReady_t 形式返回。</para>
		/// </summary>
		...
		/// <para>获取最达成的成就信息，返回一个迭代器索引，可用于获取后续最达成的成就。如果没有任何成就百分比数据（即没有调用 RequestGlobalAchievementPercentages 并等待回调），则返回 -1。</para>
		/// </summary>
		...
		/// <para>Get the info on the next most achieved achievement for the game. Call this after GetMostAchievedAchievementInfo or another GetNextMostAchievedAchievementInfo call passing the iterator from the previous call. Returns -1 after the last achievement has been iterated.</para>
		/// </summary>
		...
		/// <para>请求全球统计数据，该数据对于标记为“聚合”的统计数据可用。此调用是异步的，结果将以 GlobalStatsReceived_t 形式返回。nHistoryDays 指定要检索的日历历史天数（除了总计之外）。限制为 60。</para>
		/// </summary>
		...
		/// <para>获取聚合统计的历史数据。pData将填充每日值，从今天开始。当调用时，pData[0]将是今天，pData[1]是昨天，pData[2]是两天前，以此类推。cubData是pubData缓冲区的字节大小。返回实际设置的元素的数量。</para>
		/// </summary>
		...
		/// <para>对于具有相关进度统计的成就，使用此方法来查询该进度范围。您可能希望使用此信息在适当的里程碑完成时选择性地调用`IndicateAchievementProgress`，以向用户显示进度通知。</para>
		/// </summary>
